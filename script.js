let subjectCount = 1;
    ...
    function addCategory(category) {
      ...
      div.innerHTML = `
        <input type="number" placeholder="${category.charAt(0).toUpperCase() + category.slice(1)} Marks" min="0" max="100" id="${category}${currentSubject}">
        <input type="number" placeholder="Weightage (%)" min="0" max="100" id="${category}Weight${currentSubject}">
      `;
      container.appendChild(div);
    }

    function calculateWeightedMarks(marks, weight) {
      return (marks / 100) * weight;
    }
      ...
      for (let i = 0; i < subjectCount; i++) {
        ...
        if (document.getElementById('useDirectMarks').checked) {
          ...
          categories.forEach(cat => {
            const marks = parseFloat(document.getElementById(`${cat}${i}`).value) || 0;
            const weight = parseFloat(document.getElementById(`${cat}Weight${i}`).value) || 0;
            if (marks > 0 && weight > 0) {
              finalMarksTotal += calculateWeightedMarks(marks, weight);
              totalWeight += weight;
            }
          });
